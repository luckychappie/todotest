<template>
  <div class="todo-container px-2">
    <label class="mt-3 mb-2">
      Total completed todos: {{ completedCount }}</label
    >
    <div class="d-flex mb-3">
      ...
      />
      <button @click="addNewTodo" class="btn btn-info ms-2">
        {{ !editingTodo ? "Add" : "Update" }}
      </button>
    </div>
    ...
    <ul v-else class="list-group">
      <li
        v-for="todo in todos"
        :key="todo.id"
        class="list-group-item todo-item d-flex align-space-between"
      >
        ...
        <div>
          <button @click="editTodo(todo)" class="btn btn-sm btn-primary">
            Edit
          </button>
...
const { todos, addTodo, toggleTodo, removeTodo, getTodos, updateTodo } =
  useTodo();
